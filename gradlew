...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    # Classpath is now constructed correctly, all semicolons converted to colons
    # and paths converted to Unix format

    # JAVA_HOME is not set, so use the relative path
    [ -n "$JAVA_HOME" ] || {
        CLASSPATH=`cygpath --path --mixed "$CLASSPATH"`
        JAVACMD=`cygpath --unix "$JAVACMD"`
    }
...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    # It's very important to use proper quoting to avoid Windows cmd.exe from misinterpreting the command.
    set -- "$JAVACMD" "$@"

    # We use `"$@"` to make the arguments word-safe (platform- & shell-independent).
    exec "$@"
